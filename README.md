...
- **Real-time danmaku** — WebSocket-based bullet comment collection for highlight scoring
...
```
IDLE ──score>threshold──▶ PENDING ──score>threshold(2nd)──▶ BURST
  ▲                         │                                  │
...
# Windows (requires static/ffmpeg/win/ffmpeg.exe, see below)
npm run package:win
```
...
The app generates a default config on first launch at `~/Library/Application Support/LiveClipAI/config.yaml` (macOS) or `%APPDATA%/LiveClipAI/config.yaml` (Windows). All settings can also be changed in the Settings page of the app.

### AI Provider

Supports any OpenAI-compatible API. Configure in Settings or edit `config.yaml`:
...
|----------|-------------|--------|
| `whisper-local` | Local whisper.cpp, offline, no API needed | Default |
...
│   │   ├── scoring-engine.ts    # Multi-signal scoring
...
- Clip scoring weights are **auto-redistributed** when a signal is unavailable (e.g., no danmaku → audio+ASR split the weight).
